...
async fn test_mock_trade_execution() {
    ...
    let resp = mock
        ...
        .await
        .expect("Failed to open mock position");
    ...
    mock.close_position(deal_id, "SELL", 1.0)
        .await
        .expect("Failed to close mock position");

    // 5. Verify position is gone
    let final_positions = mock
        .get_positions()
        .await
        .expect("Failed to get final positions");
